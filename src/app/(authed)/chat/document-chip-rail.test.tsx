...
describe('DocumentChipRail detach UX', () => {
  ...
  it('shows a spinner while onDetach is in-flight', async () => {
    ...
    // Re-bind to a const so TS can narrow past the closure-assignment widening
    // (same pattern used in use-thread-documents.ts for `snapshot`).
    const settle = resolve as (() => void) | null;
    settle?.();
